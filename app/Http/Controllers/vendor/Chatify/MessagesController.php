...
class MessagesController extends Controller
{
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->extension()), $allowed)) {
                    ...
                    $error->status = 1;
                    $error->message = "Расширение файла не допускается!";
                }
            } else {
                $error->status = 1;
    ...
    public function fetch(Request $request)
    {
        ...
        // если сообщений еще нет.ить
        if ($totalMessages < 1) {
            $response['messages'] ='<p class="message-hint center-el"><span>Скажи \'привет\' и начни переписываться</span></p>';
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="message-hint"><span>Пока ничего не публиковать</span></p>',
        ], 200);
    }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->extension()), $allowed_images)) {
                    ...
                    $msg = "Расширение файла не допускается!";
                    $error = 1;
                }
            } else {
